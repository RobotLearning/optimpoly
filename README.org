...
save every image).
*** TODO Calibrate vision using robot data and images
**** TODO Align saved joint positions with video frames
**** TODO Compare the existing calibration predictions 
with robot 3d data and still balls
**** TODO Learn projection matrices with least squares
Use also RANSAC
**** TODO Make new predictions on robot data and still balls
Using the new projection matrices (e.g. invert them?)
**** TODO Record new robot data with PONG
**** TODO Predict on new PONG data and compare with old calibration
**** TODO Check for undistortion of images 
Check PvAPI for c++ function to perform distortion correction
**** TODO Learn projection matrices after distortion correction
...
** IN-PROGRESS Learning from Demonstrations
...
With the egg-hold it is possible to teach, but the shoulder is as
...
**** IN-PROGRESS Resolve the problem of high initial accelerations
Jens suggestion to reduce the initial accelerations does not work
unfortunately.
***** IN-PROGRESS Penalize the accelerations throughout
This actually generates smoothing splines. 
...
Inverse Reinforcement Learning maybe?
...
**** TODO Test ZMQ interface during human demonstrations
**** TODO Test ZMQ interface during robot serve
**** TODO Plot ball and robot data during serve
Check it in python and look for outliers.
...
Is the full planning problem with the two ball takeoff and racket
...
need to optimize repeatedly 5th order polynomials.
...
**** TODO Checkout Policy Search on Latent Space
Dimensionality Reduction on the parameters of policy search Check out
Jan's suggested paper
...
The options are model-based RL and Bayesian Optimization (BO).
**** IN-PROGRESS Which type of model?
...
**** TODO How to apply BO for Barrett WAM
...
well.
***** TODO How to do dimensionality reduction on policy params
Jan suggested performing PCA analysis.
...
**** TODO How to merge the two?
This would mean: how would observing the trajectories of the ball and
the robot help in Bayesian Optimization where we want to do black-box
optim on the reward.
...
The plan is to give videos in an unsupervised manner and predict
3d-positions.
** Semi-supervised learning on images
An autoencoder would try to generate the images with the latent space
restricted to 3 dimensions. A limited supervised-set consisting of
robot racket 3d-positions would act as constraints, pushing the 3d
latent space to physically meaningful values.

The first half of the auto-encoder trained this way would replace ball
detection and calibration + possibly distortion correction.

*** TODO How to enforce constraints during optimization
Projected gradient descent?
*** TODO Train autoencoder in keras with validation set
Enable early stopping and compare without early stopping

** Unsupervised learning on videos
Given an unlabeled repository of ball flight videos, the task is to
learn ball detection, calibration and ball model all at once.

Given an initial 3d ball position velocity and spin, one can predict
the future of the ball positions with a ball model. The calibration
function and the inverse of the ball detection would then generate the
entire video.
