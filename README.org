...
By providing options to boost_unit_test in command line
with --run_test=TEST_SUITE where TEST_SUITE is one of
SL,OPT,KIN,KF,TT,SERVE or a comma separated combination
of them we can run particular suites.
...
The problem appears to be mostly due to the json library in debug mode.
...
*** WAITING Investigate dimensionality reduction on demonstration data
The idea is to find a reduced dim. latent space
on which we can perform policy search/BO.
**** TODO Perform PCA
**** TODO Perform Lasso on the regression
As opposed to PCA, Lasso would start with many 
hypotheses corresponding to basis functions at various time points
and would hopefully prune them as some parameters are set to zero.
**** TODO Investigate autoencoders
...
and racket impact models included result in a continuous
...
Dimensionality Reduction on the parameters of policy search
Check out Jan's suggested paper
...
As opposed to GPs or neural networks, we could first try
local linear models as an extension of DMPs.
***** TODO Learn model of what?
Model for the ball take-off, ball-racket impact
or robot dynamics? Or all three? Couple the ball as part
of the state as well?
***** TODO Is the ball or the robot rest posture a context?
We have to make sure that local policy generalizes to different
ball positions and robot rest postures.
**** TODO How to apply BO for Barrett WAM
In high dimensional cases it seems important to do dimensionality
reduction.

The initial posture of the robot corresponds to the 'context'
of the problem and contextual bandits would learn a GP over
...
This would mean: how would observing the trajectories of
the ball and the robot help in Bayesian Optimization where
we want to do black-box optim on the reward.
