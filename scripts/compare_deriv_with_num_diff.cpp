...
void test_kin_deriv() {
  ...
  printf("racket_normal = [%f,%f,%f]\n", racket_normal[X], racket_normal[Y],
         racket_normal[Z]);
  ...
  /*
   * Calculate the maximum difference between numerical and exact derivative
   * matrices (big jacobian)
   */
  BOOST_TEST(calc_max_diff(deriv, num_deriv, 0, 2 * NCART, 0, 2 * NDOF) < 1e-3);
}

static void
kinematics_eq_constr(double *result, const double *x,
                     void *my_function_data) { // This is the constraint that
                                               // makes sure we hit the ball
