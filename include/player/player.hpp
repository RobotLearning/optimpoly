...
struct player_flags {
  ...
  double optim_offset =
      0.0; //!< offset after net for starting optim (if mpc is off)
  ...
  double t_reset_thresh = 0.3; //!< resetting Kalman filter after this many
                               //!< seconds pass without getting valid obs.
  double VHPY = -0.3;          //!< location of hitting plane for VHP method
  std::vector<double> weights = {
      0.0, 0.0, 0.0}; //!< hit,net,land weights for DP (lazy player)
  std::vector<double> mult_vel = {0.9, 0.8, 0.83}; //!< vel. mult. for DP
  std::vector<double> penalty_loc = {0.0, 0.23, 0.0,
...
private:
   ...
   *
   * Calculate the optimization parameters using an NLOPT nonlinear optimization
   ...
   * the frequency of updates is respected, and the ball has not passed the
   * y-limit
   *
   */
  ...
  /**
   * @brief Unfold the next desired state of the 3rd order polynomials in joint
   * space If movement finishes then the desired state velocities and
   * accelerations are zeroed.
   *
   * Multithreading : if after initial lookup, the computation in the other
   * thread terminates, then we synchonize the values between the threads and
   * compute the next desired states given these new polynomial parameters (qf,
   * qf_dot and T_hit)
   *
   */
   ...
   */
  void lookup_soln(const arma::vec6 &ball_state, const int k,
...
public:
   ...
   * @param filter_ Reference to an input filter (must be initialized in a
   * separate line).
   * @param flags Flags/options for player class, initialized with c++11 (see
   * player.hpp) or through player.cfg file (see sl_interface)
   */
   ...
   * updates the desired joint states when the optimization threads have
   * finished.
   *
   * @param ball_obs Ball observations (positions as 3-vector and update
   * status).
   * @param qact Actual joint positions, velocities, accelerations.
   * @param qdes Desired joint positions, velocities, accelerations.
   */
  void play(const ball_obs &obs, const optim::joint &qact, optim::joint &qdes);
   ...
   * @param qdes Desired joint positions, velocities, accelerations.
   */
  void cheat(const optim::joint &qact, const arma::vec6 &ballstate,
             optim::joint &qdes);
};
 ...
 * @return
 */
bool update_next_state(const optim::spline_params &poly, const vec7 &q_rest_des,
                       const double time2return, double &t_poly,
                       optim::joint &qdes);

/**
 * @brief Generate matrix of joint angles, velocities and accelerations
 */
void gen_3rd_poly(const arma::rowvec &times, const vec7 &a3, const vec7 &a2,
...
/**
 * @brief Check if the table tennis trial is LEGAL (hence motion planning can be
 * started).
 *
 ...
 *
 */
bool check_legal_ball(const arma::vec6 &ball_est, const mat &balls_predicted,
                      game &game_state);
 ...
 *
 * Bounce variable is static variable of estimate_ball_state method of player
 * class which is reset each time an incoming ball from ball gun is detected.
 *
