...
private:
	 ...
	 *
	*/
	void check_legal_land(const vec3 & ball_cand_pos,
...
public:
	 ...
	 * @param verbosity Turn ON for printing events (bounce, hit, etc.)
	 */
	TableTennis(const vec6 & ball_state,
	            bool spin = false,
	            bool verbose = false);
 ...
 */
vec calc_next_ball(const vec & xnow,
                   const double dt,
                   const void *fp);
/**
 * @brief Function that integrates a table tennis ball for an outside filter.
 ...
 * @return Next ball positions and velocities.
 */
vec calc_next_ball(const racket & robot,
                   const vec & xnow,
                   const double dt);
 ...
 * @return Next ball positions and velocities.
 */
vec calc_spin_ball(const vec & xnow,
                   const double dt,
                   const void *fp);
