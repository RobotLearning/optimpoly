...
const int INEQ_CONSTR_DIM =
    2 * NDOF + 2 * NDOF; // both strike and returning trajectories, min and max
...
/**
 * @brief Desired racket/ball positions, vels and racket normals for dt*Nmax
 * seconds.
 *
 ...
 * it has terminated (running is FALSE), then player class can update
 * trajectories.
 */
...
struct weights {
  ...
  double R_net =
      0.0; //!< weight of dist from net pos to a suitable pos above net
};
...
protected:
  ...
  mat lookup_table = zeros<mat>(
      1, OPTIM_DIM + 2 * NCART); //!< lookup table used to init. optim values
  ...
  /**
   * @brief Resting posture optimization that tries to find good resting joint
   * positions
   *
   * The resting joint positions are optimized to be
   * close to the predicted incoming ball trajectory and hitting joint states
   * with low Frobenius-norm jacobian
   */
...
public:
  ...
  double lb[2 * NDOF + 1]; //!< Joint lower limits, joint vel. lower limit and
                           //!< min. hitting time
  double ub[2 * NDOF + 1]; //!< Joint upper limits, joint vel. upper limit and
                           //!< max. hitting time
  double qrest[NDOF] = {
   ...
   *
   * If the optimization was successful, update is turned ON and the table
   * tennis player can launch/update the polynomial trajectories.
   * @return update
   */
  bool check_update();
  ...
  /**
   * @brief If the robot starts moving the optimization is notified via this
   * function
   *
   * If the robot is moving, this means last optimization was feasible, hence
   * we can initialize the new optimization from the last optimized parameter
   * values.
   * @param flag_move
   */
  ...
  /** @brief Print verbose optimization output (detailed optimization results
   * are printed) */
  ...
  /**
   * @brief Update the initial state of optimization to PLAYER's current joint
   * states.
   ...
   *
   * @param params_ Desired optimization parameters are racket and/or ball
   * values predicted or computed by player class.
   */
...
protected:
  ...
  /** @brief Initialize the optimization parameters the last optimized solution
   * values.*/
  virtual void init_last_soln(double x[]) const;
...
public:
  ...
  double dist_b2r_proj =
      1.0; //!< dist. from ball to racket proj. to racket plane
  std::vector<double> penalty_loc = {
   ...
   * @param weights weights for hitting, net and landing penalties in cost
   * function
   */
   ...
   *
   * Calculate the costs for hitting the ball, and then if land variable is set
   * to true also calculate the costs for passing it over to a desired net
   * location (x,z loc) and landing to a desired landing point (x,y loc.)
   * @return hitting and landing penalties (if land is turned on combined, else
   * only hitting)
   */
   ...
   * variable is the same (meaning optimization algorithm did not update it
   * yet).
   *
   */
   ...
   *
   */
  void calc_hit_distance(const double bp[], const double rp[],
                         const double n[]);
   ...
   */
  DefensiveOptim(const vec7 &qrest_, double lb_[], double ub_[],
                 bool land_ = true, bool lookup_ = false);
};
...
/**
 * @brief Calculate the polynomial coefficients from the optimized variables
 * qf,qfdot,T p(t) = a1*t^3 + a2*t^2 + a3*t + a4
 */
void calc_strike_poly_coeff(const double *q0, const double *q0dot,
                            const double *x, double *a1, double *a2);

/**
 * @brief Calculate the returning polynomial coefficients from the optimized
 * variables qf,qfdot and time to return constant T p(t) = a1*t^3 + a2*t^2 +
 * a3*t + a4
 */
...
/**
 * @brief Calculate the extrema candidates for each joint (2*7 candidates in
 * total) For the striking polynomial Clamp to [0,T]
 *
 */
void calc_strike_extrema_cand(const double *a1, const double *a2,
                              const double T, const double *q0,
                              const double *q0dot, double *joint_max_cand,
                              double *joint_min_cand);

/**
 * @brief Calculate the extrema candidates for each joint (2*7 candidates in
 * total) For the return polynomial Clamp to [0,TIME2RETURN]
 *
 */
...
double calc_max_acc_violation(const double x[2 * NDOF + 1],
                              const double q0[NDOF], const double q0dot[NDOF]);
 ...
 * Function that calculates a racket strategy : positions, velocities and racket
 * normal for each point on the predicted ball trajectory (ballMat) to return it
 * a desired point (ballLand) at a desired time (landTime)
 *
 */
...
/**
 * @brief Compute desired racket pos,vel,normals and/or ball positions, vels.
 * assuming spin model Function that calculates a racket strategy : positions,
 * velocities and racket normal for each point on the predicted ball trajectory
 * (ballMat) to return it a desired point (ballLand) at a desired time
 * (landTime)
 *
 * As opposed to calculating with spin-free models, this function
 * runs an optimization for each predicted ball to find desired outgoing ball
 * velocities!
 *
 */
 ...
 * @param NLOPT_FINISHED If detached, the thread will communicate that it has
 * finished
 * @param filter Filter state will be initialized after estimation
 */
void estimate_prior(const mat &observations, const mat &times,
                    const int &verbose, bool &init_ball, player::EKF &filter);
 ...
 *
 */
void estimate_ball_linear(const mat &observations, const vec &times,
                          const bool verbose, vec6 &init_est);
} // namespace optim
