...
struct des_ball_data { //Initial ball data time stamps and position observations
...
TEST(OptimTests, TestVirtualHittingPlaneOptim) {
	...
	EXPECT_TRUE(predict_hitting_point(VHPY,true,ball_pred,time_pred,filter,game_state));
	//cout << ball_pred << endl;
...
TEST(OptimTests, CheckAccuracyOfRacketCalculationsBVPUsingSpinBallModel) {
	...
	tt.calc_des_ball_out_vel(ball_land_des.head(2),time_land_des,true,ballin,ball_vel_out); // Incoming ball: ballin,
	...
	double err_norm_no_spin = norm(tt.get_ball_position() - ball_land_des,2); // error norm
	...
	optim_spin_outgoing_ball_vel(data,true,ball_vel_out); // soln. to BVP with spin

	tt.set_ball_state(join_vert(ballin.head(3),ball_vel_out));
	for (int i = 0; i < N_horizon; i++) {
		tt.integrate_ball_state(dt);
	} // resulting land. pos: tt.get_ball_position()
	double err_norm_spin = norm(tt.get_ball_position() - ball_land_des,2); //norm of error
