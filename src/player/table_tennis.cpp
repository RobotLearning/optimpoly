 ...
 *
 * Includes table tennis functions used for modelling table tennis interactions
 * between ball, table, racket (and robots, including possible opponents).
 *
 * Most of them are used in simulation mode, such as
 * the simulate_ball function.
 * Three methods are used to calculate desired racket quantities (which are
 * necessary to put here to use the parameters).
 *
...
void TableTennis::set_ball_gun(double std, int ballgun_side) {
  ...
  // using namespace std;
  vec3 ballgun = {table_center, dist_to_table - table_length - 0.2,
...
void TableTennis::symplectic_euler(const double dt, vec3 &ball_next_pos,
...
void TableTennis::check_ball_table_contact(const vec3 &ball_cand_pos,
  ...
  static const double contact_table_level =
  ...
  if ((ball_cand_pos(Y) > table_human_end) &&
      (ball_cand_pos(Y) < dist_to_table) &&
...
void TableTennis::check_ball_racket_contact(const racket &robot_racket,
  ...
  double parallel_dist_ball2racket =
      sqrt(fabs(dot(racket2ball, racket2ball) -
...
void TableTennis::check_ball_ground_contact(vec3 &ball_cand_pos,
                                            vec3 &ball_cand_vel) {
...
void TableTennis::check_legal_bounce(const vec3 &ball_cand_pos,
...
void TableTennis::calc_des_ball_out_vel(const vec2 &ball_land_des,
  ...
  balls_out_vel.row(X) =
      (ball_land_des(X) - balls_predicted.row(X)) / time_land_des;
  balls_out_vel.row(Y) =
