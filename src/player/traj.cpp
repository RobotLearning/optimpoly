...
void generate_strike(const vec7 &qf, const vec7 &qfdot, const double T,
  ...
  vec7 b3 = 2.0 * (qf - q_rest_des) / pow(time2return, 3) +
            (qfdot) / pow(time2return, 2);
  vec7 b2 = 3.0 * (q_rest_des - qf) / pow(time2return, 2) -
...
bool update_next_state(const optim::spline_params &poly, const vec7 &q_rest_des,
                       const double time2return, double &t,
                       optim::joint &qdes) {
  mat a, b;
  double tbar;
  bool flag = true;
...
void gen_3rd_poly(const rowvec &times, const vec7 &a3, const vec7 &a2,
  ...
  for (int i = 0; i < NDOF; i++) {
    Q.row(i) =
        a3(i) * pow(times, 3) + a2(i) * pow(times, 2) + a1(i) * times + a0(i);
