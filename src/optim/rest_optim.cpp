 ...
 *
 * Optimization tries to find a good resting posture close to the predicted ball
 * locations ball_pred and a given hitting state q_hit with low jacobian norm
 * (Frobenius)
 */
 ...
 */
static void intersect_ball_path(unsigned m, double *result, unsigned n,
                                const double *x, double *grad, void *data);

/*
 * Cost function for the resting posture optim
 */
static double cost_fnc(unsigned n, const double *x, double *grad, void *data);
