 ...
 */
static double nlopt_estimate_ball(const mat &obs, const vec &times,
                                  const bool verbose, vec6 &est);

/*
 * Cost function
 * Calculates also the gradient if grad is TRUE
 */
static double calc_residual(unsigned n, const double *x, double *grad,
                            void *data);
...
void estimate_prior(const mat &observations, const mat &times,
                    const int &verbose, bool &NLOPT_FINISHED,
                    player::EKF &filter) {
...
static double calc_residual(unsigned n, const double *x, double *grad,
  ...
  // residual += lambda_spin * pow(x[6] - 0.5,2) + lambda_zvel *
  // pow(x[DZ]-2.5,2) + lambda_yvel * pow(x[DY]-4.5,2);
