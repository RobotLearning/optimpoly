...
DefensiveOptim::DefensiveOptim(const vec7 &qrest_, double lb_[], double ub_[],
                               bool land_, bool lookup_)
    : FocusedOptim(), land(land_) { // FocusedOptim(qrest_, lb_, ub_) {
...
void DefensiveOptim::set_land_constr() {
  ...
  nlopt_add_inequality_mconstraint(opt, INEQ_LAND_CONSTR_DIM, land_ineq_constr,
                                   this, tol_ineq_land);
...
void DefensiveOptim::set_hit_constr() {
  ...
  nlopt_set_min_objective(opt, costfunc, this);
  nlopt_add_inequality_mconstraint(opt, INEQ_HIT_CONSTR_DIM, hit_ineq_constr,
                                   this, tol_ineq_hit);
  // nlopt_add_inequality_mconstraint(opt, INEQ_JOINT_CONSTR_DIM,
...
void DefensiveOptim::calc_times(
    const double x[]) { // ball projected to racket plane
...
double DefensiveOptim::calc_punishment() {
  ...
  double Jland = 0;
  if (land) {
    Jland = pow(x_net[X] - penalty_loc[0], 2) * w.R_net +
            pow(x_net[Z] - penalty_loc[1], 2) * w.R_net +
            pow(x_land[X] - penalty_loc[2], 2) * w.R_land +
            pow(x_land[Y] - penalty_loc[3], 2) * w.R_land;
  }
...
double DefensiveOptim::test_soln(const double x[]) const {
  ...
  joint_limits_ineq_constr(INEQ_JOINT_CONSTR_DIM, lim_violation, OPTIM_DIM, x,
                           grad, (void *)this);
  land_ineq_constr(INEQ_LAND_CONSTR_DIM, land_violation, OPTIM_DIM, x, grad,
                   (void *)this);
...
static void interp_ball(const optim_des *data, const double T, double *ballpos,
                        double *ballvel) {
