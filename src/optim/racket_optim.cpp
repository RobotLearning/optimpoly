 ...
 * is solved with spin
 *
 * We're mostly interested in the solution speed (time elapsed) using
 * optimization since an argument needs to be added to the paper
 *
 *
...
/*
 * Solve BVP for a particular predicted spinning ball's outgoing desired ball
 * velocity
 *
...
optim_des calc_spin_racket_strategy(const mat &balls_predicted,
  ...
  tennis.calc_des_ball_out_vel(ball_land_des.head(2), time_land_des, false,
                               balls_predicted, balls_out_vel);
  ...
  tennis.calc_des_racket_normal(balls_predicted.rows(DX, DZ), balls_out_vel,
                                racket_des_normal);
  tennis.calc_des_racket_vel(balls_predicted.rows(DX, DZ), balls_out_vel,
                             racket_des_normal, racket_des_vel);
...
optim_des calc_racket_strategy(const mat &balls_predicted,
  ...
  mat racket_des_normal = zeros<mat>(3, N);
  mat racket_des_vel = zeros<mat>(3, N);
  tennis.calc_des_ball_out_vel(ball_land_des, time_land_des, hack,
                               balls_predicted, balls_out_vel);
  tennis.calc_des_racket_normal(balls_predicted.rows(DX, DZ), balls_out_vel,
                                racket_des_normal);
  tennis.calc_des_racket_vel(balls_predicted.rows(DX, DZ), balls_out_vel,
...
static double calc_landing_res(unsigned n, const double *x, double *grad,
  ...
  static player::TableTennis tt =
      player::TableTennis(true, false, false); // no contact checking!
  static vec3 vel_out;
  static vec3 out_pos;
