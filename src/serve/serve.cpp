...
template <typename T>
ServeBall<T>::ServeBall(const serve_flags &sflags_, const vec7 &qinit)
    : sflags(sflags_) {
...
template <typename T> ServeBall<T>::~ServeBall() { delete opt; }

template <typename T> void ServeBall<T>::set_flags(const serve_flags &sflags_) {
  sflags = sflags_;
}
